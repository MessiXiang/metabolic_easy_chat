...
final class ChatViewModel: ObservableObject {
    ...
    func startMetabolismMode() async {
        ...
        do {
            ...
            let cloneResult = await runProcess(command: "/bin/zsh", args: ["-lc", cloneCommand], workingDirectory: parentURL, environment: environment, timeout: 300, terminalID: terminalID) { _ in }
            guard cloneResult.status == .completed else {
                showAlert("新陈代谢克隆或创建分支失败，请查看终端输出。")
                return
            }

            authorize(url: cloneURL, saveAsWorkspace: true)
            settings.metabolismSession = MetabolismSession(originalWorkspacePath: originalWorkspacePath, originalWorkspaceBookmark: originalWorkspaceBookmark, workspacePath: cloneURL.path, branchName: branchName, githubUser: ghUser, repositoryURL: repositoryURL, isGitHubReady: isGitHubReady)
            persistSettings()
            refreshWorkspaceFiles()

            appendTerminalLine("已进入新陈代谢模式。gh 已安装、已用设置中的 Token 登录，权限检查通过，提交后可自动创建 PR。", kind: .system, to: terminalID)
        } catch {
